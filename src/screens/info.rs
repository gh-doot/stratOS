...
fn show_other_info(topleft: tui::Point) {
    ...
    let other_info = [version.as_str(), "", "Press any key to exit."];
...
pub fn show_info(termsize: tui::Point) {
    ...
    let other_center = tui::Point {
        row: 0,
        col: center.col + license_center.col,
    };
    tui::move_cursor(tui::Point {
        row: 4,
        col: other_center.col - 2,
    });
    tui::set_bold(true);
    print!("Other");
    tui::set_bold(false);
    show_other_info(tui::Point {
        row: 6,
        col: center.col + 4,
    });
