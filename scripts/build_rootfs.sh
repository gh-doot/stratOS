...
arch-chroot "${1}" useradd -m stratos || die "failed to add stratos user"
arch-chroot "${1}" bash -c 'echo -e "stratos\nstratos" | passwd stratos' || die "failed to set stratos user password"
arch-chroot "${1}" bash -c "echo 'stratos ALL=(ALL:ALL) NOPASSWD:ALL' >> /etc/sudoers" || die "failed to add stratos to sudoers"
